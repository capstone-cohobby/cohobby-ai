...
2. **분쟁 사례:** {dispute_evidence_str}
   - **분쟁 사례가 있는 경우:** 분쟁 사례를 분석하여 유사 사고 방지 규칙을 생성하라.
   - **분쟁 사례가 없는 경우:** "검색된 유사 분쟁 사례가 없습니다"라는 메시지가 표시될 수 있다. 이 경우 분쟁 사례 없이도 카테고리별 가이드와 상품 상태만으로 규칙을 생성하라.
...
- **분쟁 사례가 있는 경우에만:** 분쟁 사례에서 사고 유형 추출
- **분쟁 사례가 없는 경우:** 카테고리별 가이드와 상품 상태만으로 규칙 생성
- 상태별 추가 점검 항목 도출
- 통합하여 최종 규칙 구성
</thinking>

규칙 구성:
- 카테고리별 기술적 점검 항목 포함 (필수)
- **분쟁 사례가 있는 경우에만:** 분쟁 사례 기반 방어 규칙 포함
- 상품 상태 반영 규칙 포함
- 반납/배상 관련 규칙 포함
- 말투: ~할 것, ~필수, ~금지
...
{{
  "rules": [
    "<카테고리 특화 기술적 규칙 1>",
    "<카테고리 특화 기술적 규칙 2>",
    "<분쟁 사례 기반 방어 규칙 (분쟁 사례가 있는 경우에만)>",
    "<상품 상태(condition) 반영 규칙>",
    "<반납/배상 관련 규칙>"
  ],
  "reasoning": "<왜 이 규칙을 정했는지 전문가적 관점에서 서술. 카테고리 가이드, 분쟁 사례(있는 경우), 상품 상태를 어떻게 통합했는지 명시. 분쟁 사례가 없는 경우 '분쟁 사례 없이 카테고리 가이드와 상품 상태만으로 규칙을 생성함'이라고 명시>"
}}
</JSON_OUTPUT_SCHEMA>
